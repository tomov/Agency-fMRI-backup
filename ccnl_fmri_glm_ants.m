function ccnl_fmri_glm_ants(EXPT,model,subjects,fake)
    
    % Estimate subject-level GLM on non-normalized functionals. Note: this
    % function should only be used after preprocessing data with
    % ccnl_fmri_preproc_ants.m.
    % 
    % If fake = true, only creates the SPM.mat files, without actually 
    % running the GLM. Use for quick sanity checks.
    %
    % USAGE: ccnl_fmri_glm_ants(EXPT,model,[subjects])
    ...
    for subj = subjects
        ...
        job.dir{1} = modeldir;
        S = EXPT.subject(subj);
        job.mask{1} = fullfile(S.datadir,'mask.nii');
        for i = 1:length(S.functional)
            ...
            job.sess(i).scans{1} = fullfile(S.datadir,['sru',S.functional{i}]);
