...
for s = 1:length(subjects)
    ...
    if all(size(Y) == size(mask))
        bic(s,:) = N*(log(Y(mask))) + K*log(N); %BIC for each voxel, for each subject; this ccnl_bic for each GLM (one with RPE, one with psi, one with RPE*psi, one with all 3)
        ...
        bic(s,:) = N*(log(Y(res_inds))) + K*log(N);
    end
end
